-- Migration: Verify patient ownership in reschedule_appointment
-- The RPC function previously only checked that the appointment existed and
-- was CONFIRMED. It did not verify that the appointment belonged to the
-- patient making the request, allowing a mismatched appointment ID to
-- reschedule another patient's appointment.
